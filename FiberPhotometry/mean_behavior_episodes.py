...
def read_summary_file(file_path):
    summ_file = pd.ExcelFile(file_path, engine='openpyxl')
    sheets = list(summ_file.sheet_names)
    sheet_dfs = []

    for sheet in sheets:
        df = pd.read_excel(summ_file, header=0, sheet_name=sheet, dtype=object, engine='openpyxl')
        day = int(sheet[-1])
...
def load_from_excel_summary(exp_metadata):
    ...
    x = pd.ExcelFile(join(behavior_label_path, behavior_filename), engine='openpyxl')
